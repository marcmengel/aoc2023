...
def get_handtype(hand:  String) : Integer = {
    ...
    var jokers = counts.getOrElse('J',0)
    if (jokers > 0) {
      counts = counts.-('J')
    }
    ...
    if (countl(0)._2 + jokers == 5) 
       htype=7  // five of a kind
    if (countl(0)._2 + jokers == 4) 
       htype=6  // four of a kind
    if (countl(0)._2 + jokers == 3) 
       if( countl(1)._2 == 2)
         htype=5 // full house
       else
         htype=4 // three of a kind
    if (countl(0)._2 + jokers == 2) 
       if( countl(1)._2 == 2)
         htype=3 // two pair
       else
         htype=2 // one pair
...
for (l <- lines) {
   ...
   hsl = new HandStore(get_handtype(f(0)), f(0).replace('A','Z').replace('K','Y').replace('T','B').replace('J','0'),  f(1).toInt) :: hsl
}
